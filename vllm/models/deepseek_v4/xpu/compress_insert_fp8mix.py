...
def is_sycl_compress_unsupported_error(exc: BaseException) -> bool:
    if isinstance(exc, ImportError):
        # vllm_xpu_kernels is not installed at all.
        return True
    if isinstance(exc, AttributeError):
        # The op is absent from the installed vllm_xpu_kernels build.
        return "fused_kv_compress_norm_rope_insert_sparse_attn" in str(exc)
    msg = str(exc).lower()
    return "fused_kv_compress_norm_rope_insert_sparse_attn" in msg and (
...
def compress_norm_rope_store_sycl_fp8mix(
    ...
    # The SYCL op accepts fp32 or bf16 RMSNorm weight and upcasts to fp32
    # inside the kernel (matching Triton). Only convert exotic dtypes
