...
def xpu_sparse_decode_fp8(
    ...
    # Build combined indices into the flat workspace. Width is the static
    # K_total: topk slots occupy columns [0, max_topk), swa slots occupy
    # [max_topk, K_total). No contiguous packing is needed since the attention
    # kernel masks every index < 0 at any position, so -1 gaps are skipped like
    # padding. Keeping the width and layout data-independent (no `.item()` sync,
    # no per-token loop) lets this op be captured by a CUDA/XPU graph.
