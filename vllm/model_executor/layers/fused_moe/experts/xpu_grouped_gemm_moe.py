...
class XPUGroupedGemmExperts(mk.FusedMoEExpertsModular):
    ...
    def apply(
        ...
        # Xe2 grouped GEMM consumes high-precision A, so quantized activations
        # must be dequantized here even though the op now takes a ptr_A_scale.
        ...
        if self.is_int4 or self.is_mxfp4:
            ...
            assert w1.dtype == expected and w2.dtype == expected, (
                ...
                f"w1={w1.dtype} w2={w2.dtype}"
            )
            assert self.w1_scale is not None and self.w2_scale is not None, (
                "XPUGroupedGemmExperts: packed weights need scales, else the "
                "kernel falls back to the unquantized path"
            )
