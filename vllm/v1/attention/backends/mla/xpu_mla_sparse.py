...
# Optional deepklox sparse MLA kernels; the Triton kernel is the BF16
...
try:
    ...
    from deepklox import flash_mla_with_kvcache as _dklox_flash_mla_with_kvcache

    _DEEPKLOX_SPARSE_AVAILABLE = True
except ImportError:
    _DEEPKLOX_SPARSE_AVAILABLE = False
...
class XPUMLASparseImpl(MLAAttentionImpl[XPUMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        self.use_deepklox = _DEEPKLOX_SPARSE_AVAILABLE
        self.use_fp8_kv_cache = kv_cache_dtype == "fp8_ds_mla"
        if self.use_fp8_kv_cache and not _DEEPKLOX_SPARSE_AVAILABLE:
            raise NotImplementedError(
                "The fp8_ds_mla KV cache on XPU requires the deepklox sparse "
                "decode kernel; the Triton fallback is BF16-only. Re-run with "
                "--kv-cache-dtype auto or install deepklox."
            )
        if self.use_deepklox:
            logger.info_once(
                "Using deepklox sparse MLA kernels on XPU (%s KV cache).",
                "fp8_ds_mla" if self.use_fp8_kv_cache else "BF16",
            )
        else:
            logger.info_once(
                "deepklox is not available; falling back to the Triton BF16 "
    ...
    def _forward_bf16_kv(
        ...
        if self.use_deepklox:
            ...
            # is 0 are left untouched by the deepklox kernel, and empty rows
    ...
    def forward_mqa(
        ...
        # The deepklox kernels can skip the -1 padding tail of each row when
        ...
        remapped = triton_convert_req_index_to_global_index(
            ...
            return_valid_counts=self.use_deepklox,
        )
        if self.use_deepklox:
