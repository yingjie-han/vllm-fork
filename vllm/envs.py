...
if TYPE_CHECKING:
    ...
    VLLM_XPU_ISOLATE_WORKER_DEVICES: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Experimental: restrict each mp TP worker's ZE_AFFINITY_MASK to its own
    # device before spawn, so early SYCL/L0 runtime init (e.g. from platform
    # detection at import time) can't leak a context onto device 0 from
    # workers assigned to other devices.
    "VLLM_XPU_ISOLATE_WORKER_DEVICES": lambda: bool(
        int(os.getenv("VLLM_XPU_ISOLATE_WORKER_DEVICES", "0"))
    ),
